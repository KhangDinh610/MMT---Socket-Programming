...
class Client:
	...
	def listenRtp(self):		
		"""Listen for RTP packets and reassemble fragments."""
		while True:
			try:
	...
	def updateMovieOpenCV(self, frameData):
		"""Decode JPEG data using OpenCV and display in GUI."""
		try:
			# Chuyển bytes thành numpy array
			nparr = np.frombuffer(frameData, np.uint8)
			
			# Decode JPEG bằng OpenCV (nhanh hơn PIL)
			...
			if img is not None:
				...
				# Chuyển sang PIL Image để hiển thị trong Tkinter
